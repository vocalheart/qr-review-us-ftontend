...
import { LayoutDashboard, QrCode, Star, FileText, Menu, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Sidebar({ isCollapsed, setIsCollapsed }) {
  ...
  return (
    <>
      ...
      <aside
        ref={mobileDrawerRef}
        className={`fixed left-0 top-0 h-screen w-64 bg-white border-r border-gray-100 shadow-xl z-50 flex flex-col md:hidden transition-transform duration-300 ease-in-out ${isMobileOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        {/* Mobile Header */}
      ...
      ════════════════════════════════════════ */}
      <aside
        className={`fixed left-0 top-0 h-screen bg-white border-r border-gray-100 shadow-sm z-40 hidden md:flex flex-col transition-all duration-300 ease-in-out ${isCollapsed ? "w-16" : "w-64"
          }`}
      >
        {/* Desktop Header */}
        <div className={`flex items-center h-14 sm:h-16 border-b border-gray-100 transition-all duration-300 ${isCollapsed ? "justify-center px-2" : "justify-between px-4"
          }`}
        >
          {!isCollapsed && (
